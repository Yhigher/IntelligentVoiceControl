    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_STATE_CHANGE:
          ...
          {
            char theMessageData[] = "I AM end_device\r\n";
            GenericApp_SendTheMessage(theMessageData,(byte)osal_strlen( theMessageData ) );
          }
          break;
...
void GenericApp_SendTheMessage(char * theMessageData ,byte len)
{
  ...
  GenericApp_DstAddr.addr.shortAddr = 0x0000;   //短地址（逻辑/网络）
  GenericApp_DstAddr.addrMode = afAddr16Bit;    //通信的模式  点对点
...
static void rxCB(uint8 port ,uint8 event)
{
  ...
  rxLength = Hal_UART_RxBufLen(HAL_UART_PORT_0);  //接收字符串的个数
  if(rxLength > 0)
  {
    HalUARTRead(HAL_UART_PORT_0,buf,rxLength);//读取到保持到buf中
    HalUARTWrite(HAL_UART_PORT_0,buf,rxLength);//从buf中写到电脑里面
...
void GenericApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId )
  {
   ...
   // HalUARTWrite(HAL_UART_PORT_1,(uint8 *)&len,2);
        if (len > 0) {
